...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    // float noiseLum;
    // Unity_Dither_float(1, float4(inputData.normalizedScreenSpaceUV.xy, 0, 0), noiseLum);///
    // clip(noiseLum < .1 ? -1 : 1);
    ...
    Unity_Dither_float(1, float4(inputData.normalizedScreenSpaceUV.xy, 0, 0), noiseLum);
    Unity_Distance_float4(float4(_WorldSpaceCameraPos, 0), float4(input.positionWS, 0), distance);
