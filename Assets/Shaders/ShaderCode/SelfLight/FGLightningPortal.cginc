...
float3 NoramalMapCalculate(v2f i)
{
    ...
    tangentSpaceNormal = normalize(lerp(float3(0, 0, 1), tangentSpaceNormal, _NormalIntensity * sin(i.uv.x * TAU / 2 + _Time.y * _NormalIntensitySpeed) / 5 + .2 * i.uv.y));//
...
// float3 SpecularCalculate(v2f i, float3 N)
// {
//     float3 V = normalize(_WorldSpaceCameraPos - i.wPos);
//     float3 viewReflect = reflect(-V, normalize(i.normal));
//     float mip = (1 - _Gloss) * 6;
//     return tex2Dlod(_Specular, float4(DirToRectliner(viewReflect), mip, mip)).xyz * _SpecularIntensity;
// };
...
{
    _DisplacementPower *= sin(i.uv.x * TAU / 2 + _Time.y * _DisplacementSpeed) / 5 + .2 * i.uv.y;

    i.screenPos /= i.screenPos.w;
    fixed4 tex = tex2D(_MainTex, float2(i.screenPos.x, i.screenPos.y)) * _ColorMultipler;
    ...
    // return float4(tex);
    return float4(tex * color * diffuseLight * _SurfaceColor + specularLight * diffuseIBL, 1);
}
